...
function draw() {
	...
	for (let i = 0; i < 6; i++) {
		let size = 300 - i * 50; // Grösse der Formen anpassen
		let colorFill = (i % 2 === 0) ? blau : pink; // Wechselnde Farben
		fill(colorFill);
		rect(width / 2 + abstandX * i, height / 2 + abstandY * i, size, size, radius);
	}
